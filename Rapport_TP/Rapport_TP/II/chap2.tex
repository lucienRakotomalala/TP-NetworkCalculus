...
\begin{description}
\item[$\betha^{C1}$ ] Qui est la même que $\betha^{A}$, dans le chapitre précédent, figure \ref{fig:serviceA}, page \pageref{fig:serviceA}. En effet les n\oe uds A et C ont les mêmes caractéristiques et les entrées ont les mêmes courbes d'arrivées (BAG et $s_{max}$) la courbe de service est donc identique.

\item[$\beta^{B1}$ ] Pour calculer cette courbe de service, nous avons calculer .
